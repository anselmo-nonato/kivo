...
# Exact calibrated SVG code with analytical slopes
svg_code = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="140 280 360 440" width="360" height="440">
  ...
  <!-- 2. ARROW SHAFT (Continuous ribbon rising into arrow head) -->
  <polygon points="265,490 380,350 435,395 285,550" fill="url(#arrowGrad)" />

  <!-- 3. ARROW HEAD (Sharp spearhead matching exact vertices) -->
  <!-- Tip: (472, 288), Left barb: (375, 318), Left notch: (385, 345), Right notch: (430, 390), Right barb: (472, 360) -->
  <polygon points="472,288 375,318 385,345 430,390 472,360" fill="url(#arrowGrad)" />

  <!-- 4. LOWER RIBBON ARM (Slope m=0.96, descending to rounded cap at X=472, Y=610) -->
  <!-- Top line: from (295, 435) to (442, 575)
       End cap: arc around (432, 608) through (472, 610) to (395, 646)
       Bottom line: from (395, 646) to (255, 510)
  -->
  <path d="M 295,435 
           L 442,575 
           A 36,36 0 0,1 472,610 
           A 36,36 0 0,1 395,646 
           L 255,510 
           Z" 
        fill="url(#lowerArmGrad)" />

  <!-- 5. 3D INSIDE SHADOW (Concave interior of the ribbon loop) -->
  <path d="M 295,435 
           L 238,562 
           C 225,580 238,600 255,510 
           Z" 
        fill="url(#shadowGrad)" />

  <!-- 6. 3D FRONT LIGHT-GREEN FOLD (Continuous ribbon looping over the stem) -->
  <path d="M 385,345 
           L 240,490 
...
# Load original and rendered, compute pixel overlap:
