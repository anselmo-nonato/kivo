...
async def register(req: UserRegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    cats = [
        ...
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Tarifas & Impostos", icon="receipt-text", color="#64748B"),
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Juros & Multas", icon="alert-triangle", color="#E11D48"),
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Educação", icon="graduation-cap", color="#0284C7"),
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Compras & Cuidados", icon="shopping-bag", color="#D97706"),
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Investimentos", icon="trending-up", color="#0D9488"),
        Category(id=uuid.uuid4(), workspace_id=solo_workspace.id, name="Receitas", icon="arrow-down-left", color="#00D084"),
    ]
    db.add_all(cats)
    await db.commit()
